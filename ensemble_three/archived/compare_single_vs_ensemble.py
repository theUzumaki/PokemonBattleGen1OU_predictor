"""
Compare single XGBoost vs bagging ensemble (archived).
"""
...
def main():
    ...
    }, None, single_model.predict_proba(X_test_scaled)[:, 1])
    ensemble_models = load_ensemble_models()
    ensemble_metrics, ensemble_pred, ensemble_proba = evaluate_ensemble_on_test(ensemble_models, X_test_scaled, y_test)
    out = {'single': single_metrics, 'ensemble': ensemble_metrics}
    with open(RESULTS_DIR / 'ensemble_vs_single_metrics.json', 'w') as f:
